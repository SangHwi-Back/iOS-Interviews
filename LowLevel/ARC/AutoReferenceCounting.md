...
withExtendedLifetime()는 특정 객체가 클로저 내에서 파라미터로 반영되거나 실행 혹은 사용될 경우 클로저가 끝나기 전까지는 생명주기가 끝나지 않는다. defer 처럼 사용할 수도 있는 것이다. 하지만 이 방법보다는 아래의 Redesign 을 추천하는 경우가 많은데, 유지보수 비용 상승과 코드 가독성 문제 때문이다.

참조할 객체에 대해 다시 디자인 하는 방법은 상황에 따라 여러가지가 될 수 있다. 예를 들어 Weak/Unowned 로 참조해야 하는 프로퍼티만을 따로 관리하는 객체를 만든다. Reference Cycle 을 일으킨 객체들은 새로 만들어진 객체를 참조하기 때문에 ref_count 관리가 수월해진다. Reference Cycle 을 관리할 때 새로 생성된 객체만을 관리하면 되는 것이다.
...
하지만 Strong 참조가 0이 되면 객체는 deinitialized 상태가 된다. 이 상태(String ref_count 는 0, Weak ref_count 는 1 이상)만으로는 메모리에서 객체가 해제되는 것을 보장하지 않기 때문에 좀비 상태로 남아버리는, 즉 Memory 누수가 발생하는 문제가 있었다.

참고로 이 좀비가 Weak 참조하는 변수에 의해 참조되었다면 '런타임에' Weak 참조가 다시 로드 되어야만 메모리에서 해제 관련 알고리즘이 작동된다. Weak 참조가 실제 로드되어야 좀비 객체가 있는지 체크되는 것이다. 사용이 끝나거나, 실행 주기가 끝난 시점에 객체를 메모리에서 해제된다고 여겨졌던 것보다 까다로운 내용이 발견되었다.

그래서 약한 참조는 Side-Table 이라는 별도의 메모리를 참조하기 시작한다. 여기서 Strong/Unowned 참조는 객체 자체를 참조한다. Side-Table 은 객체의 추가적인 정보를 저장하기 위한 분리된 메모리이다.

이제 Weak 참조는 별개의 문제인 상태로, 강한 참조가 0이 되면 객체는 어김없이 deallocate 된다. 좀 더 효율적인(가차없는) 메모리 관리가 진행되는 것이다.

사실 Side Table 이 발생하는 경우는 Weak 참조만이 아니다. 아래는 Side Table 이 발생하는 조건을 나열한 것이다.
...
앞에서 언급했듯이, Weak 참조에서 바뀐 점은 **Strong/Unowned 참조처럼 객체를 직접 참조하는 것이 아닌 객체의 Side Table 을 참조한다는 것이다.**
